...
public class RecursiveDescentParser
{
    ...
    private Expression  CheckAllSituations(string next, string? previous)
    {
        ...
        if (_position == 0 && _operations.Contains(next))
            ...
            message = MathErrorMessager.OperationBeforeParenthesisMessage(previous ?? string.Empty);
        else if (previous == "(" && _operations.Contains(next))
            message = MathErrorMessager.InvalidOperatorAfterParenthesisMessage(next);
        else if (_operations.Contains(previous) && _operations.Contains(next))
            message = MathErrorMessager.TwoOperationInRowMessage(previous ?? string.Empty, next);
        else if (Numbers.IsMatch(next))
            message = MathErrorMessager.NotNumberMessage(next);
        else if (_operations.Contains(previous) && next == string.Empty)
            message = MathErrorMessager.EndingWithOperation;
        else
            message = MathErrorMessager.UnknownCharacterMessage(next.ToCharArray()[0]);
        throw new ArgumentException(message);
    }
}